...
#
# Usage: ./scripts/agent-promote.sh <slug> [flags]
#
# Flags:
...
if [[ "$BASE_REPO" == *"/.worktrees/dispatch-"* ]]; then
...
resolve_worker_branch() {
  ...
  local matches count
  matches=$(git -C "$BASE_REPO" for-each-ref --format='%(refname:short)' "refs/heads/dispatch/${SLUG}-*")
  count=$(printf '%s' "$matches" | grep -c '^' || true)

  if (( count == 0 )); then
    die "no worker branch found for slug '${SLUG}' (searched refs/heads/dispatch/${SLUG}-*)"
  fi
  if (( count > 1 )); then
