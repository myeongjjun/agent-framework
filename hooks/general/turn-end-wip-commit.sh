...
# turn while inside a sib/dispatched worker worktree. Captures every iteration
# so `agent-promote.sh` can later squash + merge a clean single commit into
# the base branch.
# @hook event: Stop
# @hook timeout: 30
#
# Fires only when the agent's cwd resolves to a worker worktree, detected via:
#   - sib branch       `sib/<slug>`        (current — `sib spawn --worktree`)
#   - legacy branch    `dispatch/<slug>-(claude|codex)-<idx>`
#   - sib worktree dir `<repo>-<slug>` under `.local/share/sib/worktrees/`
#   - legacy worktree  `.worktrees/dispatch-<slug>-(claude|codex)-<idx>`
...
# sib branches are `sib/<slug>`; legacy orchestrator branches are
# `dispatch/<slug>-(claude|codex)-<idx>`.
slug=""
if [[ "$branch" =~ ^sib/(.+)$ ]]; then
  slug="${BASH_REMATCH[1]}"
elif [[ "$branch" =~ ^dispatch/(.+)-(claude|codex)-([0-9]+)$ ]]; then
  slug="${BASH_REMATCH[1]}"
fi
if [[ -z "$slug" ]]; then
  case "$worktree_root" in
    */.local/share/sib/worktrees/*)
      # sib worktree dir is `<repo>-<slug>`; the slug is everything after the
      # first `-` is ambiguous (repo names contain dashes), so prefer the
      # branch match above. Fall back to the full dir tail only if branch
      # detection failed — agent-promote resolves the real branch by slug.
